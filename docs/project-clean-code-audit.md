...
## P0 - Fix Runtime / User-Facing Risks

### 1. Add the missing contact API route

`src/components/forms/contact-form.tsx` submits to:

```ts
fetch("/api/contact", ...)
```

But `src/app/api/contact/route.ts` does not exist. Current API routes are only:
...
- Reuse `validateContactForm`, `deliverLead`, and `isDuplicateLead`.
- Keep the honeypot `website` behavior consistent with `service-request`.
- Add `CONTACT_WEBHOOK_URL` or intentionally reuse a shared lead webhook.

Why this matters:

- The contact page can render and build successfully while the primary form still fails at runtime.

### 2. Decide which contact sections should actually render

`src/features/contact/components/index.ts` exports:
...
But `src/app/[locale]/contact/page.tsx` currently renders only:
...
- Either render the full approved design sequence: hero, hub/form, location, features, CTA.
- Or remove/export less if those components are intentionally unused.

Why this matters:

- The codebase currently suggests a full page redesign exists, but the route does not mount all of it.
